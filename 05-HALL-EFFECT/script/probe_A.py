...
plt.errorbar(T[:-4],1/abs(R_hall[:-4]),label=r"$|R_\mathrm{Hall}|^{-1}$ / $\mathrm{m}^3\,\mathrm{C}^{-1}$",xerr=1.5,yerr=R_hall_err[:-4]/R_hall[:-4]**2,ls="none",capsize=2)
plt.errorbar(T[-4:],1/abs(R_hall[-4:]),xerr=1.5,yerr=[R_hall_err[-4:]/R_hall[-4:]**2,[0,0,0,0]],capsize=2,c="C0",ls="none")
plt.errorbar(T[-4:],1/abs(R_hall[-4:]),xerr=1.5,yerr=[300,400,500,600],capsize=2,c="C0",ls="none",uplims=True)
...
E_gap_300, E_gap_300_err = E_gap*10 - 4e-4 * 300, E_gap_err*10
A, A_err = np.exp(popt[1]), np.exp(popt[1]) * np.sqrt(pcov[1][1])
ccc_300 = A * 300**(2/3) * np.exp(-E_gap_300/(2 * 0.08617e-3 * 300))
ccc_300_err = np.sqrt( (ccc_300 * A_err/A)**2 + (ccc_300 * 1/(2 * 0.08617e-3 * 300) * E_gap_300_err )**2 )

print()
print(f"Germanium Bandgap: E =  {E_gap*10:.3f} +- {E_gap_err*10:.3f} eV")
print(f"at 300 Kelvin: E_300 =  {E_gap_300:.3f} +- {E_gap_300_err:.3f} eV")
print(f"Y axis intercept:  y = {popt[1]:.3f} +- {np.sqrt(pcov[0][0])*1e-1:.3f} eV")
print(f"CCC at 300 K: CCC = {ccc_300:.3e} +- {ccc_300_err*1e-1:.3e} 1/cm3")
